...
import com.lovebuy.cas.util.CommUtil;
...
public class AuthenticationViaFormAction {
    ...
    public final void doBind(final RequestContext context, final Credentials credentials) throws Exception {
        ...
        // 新增用于数据初始化
        String ip = CommUtil.getRemoteHost(request);
        UsernamePasswordCredentials uc = (UsernamePasswordCredentials)credentials;
        uc.setLoginIp(ip);
        if(uc.getError_count() == null){
        	uc.setError_count(0);
        }
        if(org.apache.commons.lang.StringUtils.isBlank(uc.getOld_loginIp())){
        	uc.setOld_loginIp(ip);
        } 
        if(!uc.getOld_loginIp().equals(uc.getLoginIp())){
        	uc.setError_count(0);
        	uc.setOld_loginIp("");
        }else{
        	uc.setOld_loginIp(ip);
        }
        if(uc.getUsername().trim().equals("用户名")){
        	uc.setUsername("");
        }
        uc.setRed_flag(0);
        //end
